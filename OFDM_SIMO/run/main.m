...
for i = 1:length(Eb_No)
  ...
  for k = 1:frames_num
    ...
    for j = 1:ofdm_synbols_per_frame
        ...
        [x_Rx, Y_combined] = Rx(M, L, y_Tx1, y_Tx2, x_QAM_modulated, h1, h2, fft_size, No_vect(i));

        %[x_Rx, Y_combined] = Rx(M, L, y_Tx1, y_Tx2, x_QAM_modulated, h1, h2, fft_size, No_vect(i));

        %[x_Rx, Y_combined] = Rx(M, L, y_Tx1, y_Tx2, x_QAM_modulated, h1, h2, fft_size, No_vect);
  ...
  Pe_vect(i) = Pe_avg_frames_per_frame/frames_num;

  % Plot the constellation diagram for each SNR 
   %scatterplot(Y_combined);
